...
public class DAODuenoVivienda {
	 ...
	 * </br>
	 * @param duenovivienda hotal que se desea ingresar 
	...
	{
		System.out.println("addDueno");
		String sentencia = String.format("INSERT INTO %1$s.DUENOVIVIENDA (ID, LOGIN, PASSWORD, NOMBRE, CUENTABANCARIA, CORREO, IDENTIFICACION, DOCUMENTOS) "
				+ "VALUES (%2$s, '%3$s', '%4$s', '%5$s', '%6$s', '%7$s', '%8$s','%9$s')", 
				USUARIO,
				duenovivienda.getId(),
				duenovivienda.getLogin(),
				duenovivienda.getPassword(),
				duenovivienda.getNombre(),
				duenovivienda.getCuentaBancaria(),
				duenovivienda.getCorreo(),
				duenovivienda.getDocumento(),
	 ...
	 * </br>
	 * @param id id del duenovivienda del cual se quiere obtener la informacion
	 * @return un ResultSet que contiene la informacion del Duenovivienda que cumple los criterios de la sentencia SQL
	...
	public ResultSet findDuenoViviendaById (Long id) throws SQLException, Exception
	{
		String sentencia = String.format("SELECT * FROM %1$s.DUENOVIVIENDA WHERE ID = %2$s", USUARIO, id); 
		PreparedStatement prepStmt = conn.prepareStatement(sentencia);
		recursos.add(prepStmt);
	...
	/**
	 * Metodo que elimina un DuenoVivienda que tenga el id que llega por parametro de la base de datos
	 * <br>
	 * <b>Pre:</b> la conexion debe haber sido inicializada
	 * </br>
	 * @param id Id del DuenoVivienda que se desea eliminar
	...
	public void deleteDuenoVivienda(Long id) throws SQLException, Exception
	{
		String sentencia = String.format("DELETE FROM %1$s.DUENOVIVIENDA WHERE ID = %2$d", USUARIO, id); 
